...
class Cave():
    ...
    def moveSand(self):
        if self.currentSand[1] == self.lowestRock+1:
            return False
        elif not self.currentSand[1]+1 in self.rocks[self.currentSand[0]] and not self.currentSand[1]+1 in self.sand[self.currentSand[0]]:
    ...
    def checkSand(self):
        if not self.moveSand():
            if self.currentSand == (500,0):
                return True
            else:
                self.sand[self.currentSand[0]].add(self.currentSand[1])
                self.countSand += 1
                print(f"ADDING SAND: {self.currentSand} - NOW {self.countSand} SAND")
                self.newSand()
                return False
...
print(cave.lowestRock)
while True:
    if cave.checkSand(): break
print(f"TOTAL SAND IS: {cave.countSand+1}")